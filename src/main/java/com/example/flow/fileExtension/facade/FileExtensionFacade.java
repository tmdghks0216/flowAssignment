...
public class FileExtensionFacade {
    ...
    public statusCodeEnum insetCustomExtension(FileExtension fileExtension) {
        // 개수 제한 체크
        int currentCount = fileExtensionService.countCustomExtension(fileExtension);

        if(currentCount >= 200){
            log.warn("커스텀 확장자 개수 초과");
            return statusCodeEnum.LIMIT_EXCEEDED;
        }

        //중복체크
        if(fileExtensionService.isExistsExtension(fileExtension)){
            log.warn("커스텀 확장자 중복 추가 불가");
            return statusCodeEnum.DUPLICATE_EXTENSION;
        }
        fileExtensionService.insertFileExtension(fileExtension);
